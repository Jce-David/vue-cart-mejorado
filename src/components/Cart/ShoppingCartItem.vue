...
import { useImageCartStore } from '@/stores/ImageCart';

export default {

    data(){
        return { 
            stateColor: useUniqueStore(),
            stateColorCart: useImageCartStore()
        }
    },
    props: {
        detail: {
    ...
    computed: {
        selectedColorIndex() {
            return  this.stateColorCart.selectedColorImageIndex
        },
        imageShopCart() {
            console.log(this.detail.product.images);
            const selectedColorIndex = this.selectedColorIndex; // Obtener el índice de color seleccionado para el producto, o 0 si es null o undefined

return this.detail.product.images[selectedColorIndex];
        },
        productImageUrl() {
            return this.detail.product.images[2] 
        },
        subtotal() {
...
<template  >
    <div class="mt-6 ml-2 ">
        <div class=" text-center " style="display: flex; justify-content: space-between; align-items: center;">
        ...
        <v-row>
            ...
            <v-col :cols="7" :xs="7" :sm="7" :md="7" :xl="7">
                <div>
                    <p class="  text-resumen  "> {{ detail.product.name }}</p>
                    ...
                    <button class=" mt-4 text-resumen" @click="deleteProduct(detail.product.id, detail.product.talla)">
                        Eliminar </button>
